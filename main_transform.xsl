...
<!-- Additional variables for translation helper tool -->

<!-- if you want to open pootle on specific language, change from $doclang to your target. -->
<xsl:variable name="target_lang" select="$doclang"/>
<!-- example: <xsl:variable name="target_lang" select="'ja'"/> -->
...
<xsl:template name="namepart">
    <xsl:param name="z"/>
    <xsl:variable name="a"><xsl:call-template name="splitpath"><xsl:with-param name="locname" select="$z"/></xsl:call-template></xsl:variable>
    <xsl:variable name="b"><xsl:call-template name="splitpath"><xsl:with-param name="locname" select="$a"/></xsl:call-template></xsl:variable>
    <xsl:variable name="c"><xsl:call-template name="splitpath"><xsl:with-param name="locname" select="$b"/></xsl:call-template></xsl:variable>
    <xsl:variable name="d"><xsl:call-template name="splitpath"><xsl:with-param name="locname" select="$c"/></xsl:call-template></xsl:variable>
    <xsl:call-template name="splitpath"><xsl:with-param name="locname" select="$d"/></xsl:call-template>
</xsl:template>

<xsl:template name="splitpath">
    <xsl:param name="locname"/>
    <xsl:choose>
        <xsl:when test="contains($locname,'/')">
            <xsl:value-of select="substring-after($locname,'/')"/>
        </xsl:when>
        <xsl:otherwise>
...
<xsl:template match="ahelp">
	<span class="avis"><xsl:apply-templates /></span><span style="font-size: xx-small;">[ahelp]</span>
...
<xsl:template match="bookmark">
    ...
    <xsl:if test="starts-with(@branch,'index')">
        <xsl:apply-templates />
        <span style="font-size: xx-small;">[index]</span>
        <xsl:call-template name="callsearcher">
            <xsl:with-param name="id" select="@id"/>
            <xsl:with-param name="oldref" select="@oldref"/>
        </xsl:call-template>
    </xsl:if>
</xsl:template>
...
<xsl:template name="insertpara">
	...
	<p class="{$role}" id="{@id}"><xsl:apply-templates /> 
        <xsl:call-template name="callsearcher">
            <xsl:with-param name="id" select="@id"/>
            <xsl:with-param name="oldref" select="@oldref"/>
        </xsl:call-template>
...
<xsl:template name="callsearcher">
    ...
    <xsl:choose>
        <xsl:when test="string-length($oldref)=0">
            <a style="font-size: xx-small;" href="{$searchprefix}{$fname}#{$id}.help.text">[<xsl:value-of select="$id"/>]</a>
        </xsl:when>
        <xsl:otherwise>
            <a style="font-size: xx-small;" href="{$searchprefix}{$fname}#{$id}.{$oldref}.help.text">[<xsl:value-of select="$id"/>]</a>
        </xsl:otherwise>
    </xsl:choose>
